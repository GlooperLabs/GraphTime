...
class GroupFusedGraphLasso(DynamicGraphLasso):
    ...
    def degrees_of_freedom(self,thresh):
        ...
        # FIXME: The conditional below never triggers, perhaps due to thresholding
        # order in the algorithm?
        for t in range(T-1):
            if (np.linalg.norm(GammaOD[t],ord='fro') > self.lambda2):
                # Compute complete J
                sk = np.linalg.norm(GammaOD[t].flatten()>thresh,ord=0)
                cor = (self.lambda2 * 
        ...
        # block?
        dof = dof + np.linalg.norm(U[0].flatten()>thresh,ord=0)
